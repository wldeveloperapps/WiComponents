...
| Capacidad Wi (objetivo) | Origen Prime (referencia) | Dominio                     | Cubo     | Estado   | Notas                                                                                                                                                                                                                                                                            |
| ----------------------- | ------------------------- | --------------------------- | -------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Table                   | `table`                   | `data-display`              | base     | done     | `@wiloc/ui/data-display` (`wi-table`): sort, filtros, page, visibility, row actions; `showFrom` por columna (`always`/`sm`/`md`/`compact`/`lg`, default `compact` = 960px de contenedor); chevron/tarjeta si hay columnas fuera de la fila; `compact` override. Reorder → futuro |
| Card                    | `card`                    | `data-display`              | base     | done     | `@wiloc/ui/data-display` (`wi-card` + parts); `size` sm / md / none (flush); host `py`+`gap`; px en header/content; `class` merge (consumidor gana)                                                                                                                              |
| Chip / Tag              | `chip` / `tag`            | `data-display`              | base     | done     | `@wiloc/ui/data-display` (`wi-chip`); tags `sm` + selección `clickable`/`selected`; removable; `radius` `control` \| `full`; proyección de icono                                                                                                                                 |
